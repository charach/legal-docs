...
아내 페이지 세 곳에만 「월레스와 그로밋」 GIF를 한 컷씩 둔다. 절제 기조와는
어긋나는 요소라 **작게(168px), 페이지당 하나, 흑백 배경 위에** 올려 균형을 맞췄다.

- `study_calendar` — 그날 상태에 따라 바뀐다 (쉬는 날 / 진행 중 / 다 끝냄)
- `ai_grad_prep`, `interview_prep` — 각 한 컷 고정
- 목록은 `data/gromit.json`. Giphy(Aardman 공식 채널)에서 **핫링크**하므로
  저장소에 파일을 두지 않는다. 네트워크가 없으면 이미지만 비고 레이아웃은 유지된다
  (`aspect-ratio` 지정)

늘리고 싶어지면 참을 것. 하나가 넘어가는 순간 페이지가 산만해진다.
